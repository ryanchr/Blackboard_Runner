...
public class eatRainbowBox : MonoBehaviour {
	...
	void Start () {
		...
		if(random==0)
			Instantiate(paintBox,new Vector3(new_x+0.5f,new_y+6.5f,0),transform.rotation);
		else
			Instantiate(paintBox,new Vector3(new_x+1.6f,new_y+3.4f,0),transform.rotation);
	}
